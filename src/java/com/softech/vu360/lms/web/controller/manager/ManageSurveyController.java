...
public class ManageSurveyController extends VU360BaseMultiActionController{

	private static final Logger log = Logger.getLogger(ManageSurveyController.class.getName());
	...
	public ManageSurveyController(Object delegate) {
		super(delegate);
	}
	...
	protected void onBind(HttpServletRequest request, Object command, String methodName) throws Exception {
		...
		if( command instanceof ManageSurveyForm ) {
	...
	public ModelAndView displaySurveys(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		return new ModelAndView(defaultManageSurveysTemplate);
	}
	...
	public ModelAndView searchSurvey( HttpServletRequest request, HttpServletResponse response, 
		...
		// Added By Marium Saud : To resolve issue LMS-20857
				...
				if( sortDirection.equalsIgnoreCase("0") ) {
					for( int i = 0 ; i < surveyList.size() ; i ++ ) {
						for( int j = 0 ; j < surveyList.size() ; j ++ ) {
							if( i != j ) {
								Survey survey1 = surveyList.get(i);
								Survey survey2 = surveyList.get(j);
	...
	public ModelAndView deleteSurvey(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(distributor !=null && ((VU360UserAuthenticationDetails)SecurityContextHolder.getContext().
				getAuthentication().getDetails()).getCurrentMode().equals(VU360UserMode.ROLE_LMSADMINISTRATOR)){
			/**Added by Marium saud : Line was added because distributor object is taken from current user where owner id is 'null' so to have owner id and ownertype 'customer.initializeOwnerParams()' is called*/
			distributor.initializeOwnerParams();
			surveyList = (List<Survey>) surveyService.getAllSurvey(distributor);
		}else{
			/**Added by Marium saud : Line was added because customer object is taken from current user where owner id is 'null' so to have owner id and ownertype 'customer.initializeOwnerParams()' is called*/
	...
	/**
	 * handler methods for showing Create Training Plan Analysis
	 */
	 ...
	 */
	 public ModelAndView questionView(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		 ManageSurveyForm form = (ManageSurveyForm)command;
		 flag=false;
		 return new ModelAndView("redirect:mgr_manageSurveys.do?method=showQuestionView&sid="+form.getSid()); 
	 }
     ...
     for(SurveyQuestion surveyQuestion : surveyQuestions){
    	 if(surveyService.getSurveyResultAnswerBySurveyQuestionId(surveyQuestion.getId())!=null){
             ...
             surveyQuestion.setAnsFlag(1);
      }
      else if( surveyQuestion instanceof AggregateSurveyQuestion ){
                   	AggregateSurveyQuestion question = (AggregateSurveyQuestion)surveyQuestion;
          	...
          	for (AggregateSurveyQuestionItem agrQuestion : aggregateSurveyQuestionItem) {
          		if(surveyService.getSurveyResultAnswerBySurveyQuestionId(agrQuestion.getQuestion().getId())!=null){
          			countAnswer++;
          		}
  			}
          	if(countAnswer==0)
          		surveyQuestion.setAnsFlag(0);
          	else
          		surveyQuestion.setAnsFlag(1);
          }
      else{
	 ...
	 */
	public ModelAndView addQuestionView(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		Map<Object, Object> context = new HashMap<Object, Object>();
		context.put("surveyQuestionTypes", SURVEY_QUESTION_TYPES);
	...
	/**
	 * setup the question type in the popup for adding questions
	 * and display the popup for the add question.
	 * Also used to display the other add Q forms when 
	 * the question type is changed on the same page
	 */
	...
	public ModelAndView saveQuestionInList(HttpServletRequest request, HttpServletResponse response, 
		...
		ManageSurveyValidator validator = (ManageSurveyValidator)this.getValidator();
		validator.validateQuestion(manageSurveyForm, errors);
		if(errors.hasErrors()){
			Map<Object, Object> context = new HashMap<Object, Object>();
			context.put("customResponceTypes", CUSTOM_RESPONCE_TYPES);
			context.put("resNumber", NUMBER_OF_CUSTOM_RESPONCES);
		...
		if(questionType.equalsIgnoreCase(SURVEY_QUESTION_MULTIPLE_CHOICE_MULTIPLE_SELECT)){
			...
			PersonalInformationSurveyQuestion question = manageSurveyForm.getCurrentPersonalInformationQuestion();
			question.setRequired(manageSurveyForm.isSurveyQuestionRequired());
			List<AvailablePersonalInformationfield> personalInfoFields = new ArrayList<AvailablePersonalInformationfield>();
			int counter = 0;
			for (ManagePersonalInformation item : manageSurveyForm.getMngPersonalInfos()) {
				if (item.isSelected()) {
			...
			question.setText(manageSurveyForm.getSurveyQuestionText());
			manageSurveyForm.setCurrentPersonalInformationQuestion(question);
			manageSurveyForm.getSurveyQuestions().add(question);
		} else if ( questionType.equalsIgnoreCase(SURVEY_QUESTION_CUSTOM) ) {
			
			orderFlag=false;
			int noOfItems = manageSurveyForm.getAggregateSurveyQuestionItem().size();
			List<AggregateSurveyQuestionItem> aggregateSurveyQuestionItem = new ArrayList<AggregateSurveyQuestionItem>();
			AggregateSurveyQuestion question = manageSurveyForm.getCurrentCustomSurveyQuestion();
			question.setRequired(manageSurveyForm.isSurveyQuestionRequired());
			question.setText(manageSurveyForm.getSurveyQuestionText());
			if(manageSurveyForm.getEditableQuestionId()!=0){
			...
			manageSurveyForm.setCurrentCustomSurveyQuestion(question);
			manageSurveyForm.getSurveyQuestions().add(question);
			...
			if(deleteCustomQuestion==1){
				...
				loopIndex=manageSurveyForm.getCustomQuestionResponceTypes().size();
			}
			
			for( int i = 0 ; i < loopIndex ; i ++ ) {
				String resType = manageSurveyForm.getCustomQuestionResponceTypes().get(i);
				String label = manageSurveyForm.getResponceLabels().get(i);
				String choices = manageSurveyForm.getAnswerChoices().get(i);
				
				//boolean isMulti = manageSurveyForm.getIsMultiline().get(i);
				boolean isRequired = manageSurveyForm.getResRequired().get(i);
				if( resType.equalsIgnoreCase(SURVEY_CUSTOM_RESPONCE_MULTIPLE_SELECT) ) {
					...
					if(manageSurveyForm.getEditableQuestionId()!=0 && noOfItems>0){
                    	surveyQun.setDisplayOrder(manageSurveyForm.getAggregateSurveyQuestionItem().get(i).getDisplayOrder());
                    	aggrSurveyQuestion.setDisplayOrder(manageSurveyForm.getAggregateSurveyQuestionItem().get(i).getDisplayOrder());
        				}
                    else{
			...
			manageSurveyForm.setAggregateSurveyQuestionItem(aggregateSurveyQuestionItem);
			
		}
		
		Survey survey = surveyService.loadForUpdateSurvey(manageSurveyForm.getSid());
		
		for(SurveyQuestion surveyQuestion:manageSurveyForm.getSurveyQuestions()) {
		...
		manageSurveyForm.setSurveyQuestionRequired(false);
		...
		//return new ModelAndView(redirectTemplate,"context",context);
		return new ModelAndView("redirect:mgr_manageSurveys.do?method=showQuestionView&sid="+manageSurveyForm.getSid());
	}
...
//				sortColumnIndex = session.getAttribute("sortColumnIndex").toString();
					...
					if( sortDirection.equalsIgnoreCase("0") ) {
						for( int i = 0 ; i < courses.size() ; i ++ ) {
							for( int j = 0 ; j < courses.size() ; j ++ ) {
								if( i != j ) {
									SurveyCourse c1 = courses.get(i);
									SurveyCourse c2 = courses.get(j);
	...
	public ModelAndView searchCourses(HttpServletRequest request, HttpServletResponse response, 
		...
		if( survey.getOwner().getOwnerType().equalsIgnoreCase(Customer.CUSTOMER) ) {
			...
			if( searchType.equalsIgnoreCase("simplesearch") ) {
				...
				for(Course selectedCourse : allCourseList){
					//if(manageSurveyForm.getSearchCourseID()!= "" && selectedCourse.getId()== Long.parseLong(manageSurveyForm.getSearchCourseID())){
					// LMS-15234
					if(StringUtils.isNotEmpty(manageSurveyForm.getSearchCourseID()) 
							&& manageSurveyForm.getSearchCourseID().equalsIgnoreCase(selectedCourse.getBussinesskey())){
						courseList.add(selectedCourse);
					}else if(StringUtils.isNotBlank(selectedCourse.getCourseTitle()) 
							&& selectedCourse.getCourseTitle().toLowerCase().contains(manageSurveyForm.getSearchCourseName().toLowerCase())){
						if(StringUtils.isEmpty(manageSurveyForm.getSearchCourseID())){
							courseList.add(selectedCourse);
						}
					}
					if(StringUtils.isEmpty(manageSurveyForm.getSearchCourseID()) && StringUtils.isBlank(manageSurveyForm.getSearchCourseName()) ){
						courseList.add(selectedCourse);
					}
				}
		...
		if( CollectionUtils.isNotEmpty(courseList)) {
			Set<Course> uniqueSet = new HashSet<Course>(courseList);
				...
				if( sortDirection.equalsIgnoreCase("0") ) {
					for( int i = 0 ; i < surveyCourseList.size() ; i ++ ) {
						for( int j = 0 ; j < surveyCourseList.size() ; j ++ ) {
							if( i != j ) {
								SurveyCourse c1 = surveyCourseList.get(i);
								SurveyCourse c2 = surveyCourseList.get(j);
								if( c1.getCourse().getCourseTitle().toUpperCase().compareTo
										(c2.getCourse().getCourseTitle().toUpperCase()) > 0 ) {
	...
	public ModelAndView editQuestion(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		List<SurveyQuestion> getSurveyQuestion = surveyService.getSurveyQuestionsBySurveyId(form.getSid());
		for (SurveyQuestion surveyQuestion : getSurveyQuestion) {
		...
		context.put("questionId", Long.valueOf(form.getEditableQuestionId()));
		if(selectedSurveyQuestion instanceof MultipleSelectSurveyQuestion){
			...
			//Added these lines to set 'surveyAnswer' when question will be open in eidt mode to avoid 'Lazy Initlaiziation Exception for surveyAnswer in SurveyQuestion'
			answerLines = this.getAnswer(question.getSurveyAnswers());
			question.setSurveyAnswerLines(answerLines);
	...
	public ModelAndView showResponseSummaryView(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		log.debug("in showResponseSummaryView");
		ManageSurveyForm form = (ManageSurveyForm)command;
	...
	private SurveySectionVO createEmptySection(SurveySection surveySection) {
		log.debug("creating section " + surveySection.getId() + " " + surveySection.getName());
		// setting section
		...
		for (SurveySection childSurveySection : childrenSections) {
			log.debug("creating child section " + childSurveySection.getId() + " " + childSurveySection.getName());
		...
		if(frameworkMappings!=null && frameworkMappings.size()>0) {
			...
			for (SurveySectionSurveyQuestionBank surveySectionSurveyQuestionBank : frameworkMappings) {
				...
				log.debug("creating framework " + framework.getId() + " " + framework.getName());
				...
				for (SurveyQuestion surveyQuestion : questions) {
					log.debug("creating question " + surveyQuestion.getId() + " " + surveyQuestion.getText());
					...
					for (SurveyAnswerItem surveyAnswer : surveyAnswers) {
						log.debug("creating answer " + surveyAnswer.getId() + " " + surveyAnswer.getLabel());
	...
	private SurveyAnalysis doAnanyzeSurvey(Long surveyId) {
		log.debug("analyzing survey with id " + surveyId);
		Survey survey = surveyService.getSurveyByID(surveyId);
	...
	public ModelAndView showResponseSummarySectionView(HttpServletRequest request, HttpServletResponse response, ManageSurveyForm form, BindException errors) throws Exception {
	...
	public ModelAndView showSurveyAnalizeForIndividuals(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
				...
				if( sortDirection.equalsIgnoreCase("0") ) {
					for( int i = 0 ; i < surveyResultList.size() ; i ++ ) {
						for( int j = 0 ; j < surveyResultList.size() ; j ++ ) {
							if( i != j ) {
								SurveyResult result1 = surveyResultList.get(i);
								SurveyResult result2 = surveyResultList.get(j);
								if( result1.getSurveyee().getName().toUpperCase().compareTo
										(result2.getSurveyee().getName().toUpperCase()) > 0 ) {
									SurveyResult tempLG = surveyResultList.get(i);
									surveyResultList.set(i, surveyResultList.get(j));
									surveyResultList.set(j, tempLG);
								}
							}
						}
					}
		...
		return new ModelAndView(surveyAnalizeIndividualTemplate, "context" , context);
	}
	...
	public ModelAndView showSurveySectionAnalizeResponse(Survey survey, HttpServletRequest request, ManageSurveyForm form, List<SurveyResultAnswer> surveyResultAnswers, Learner learner) {
		...
		context.put("surveyResultAnswers", surveyResultAnswerMap);
		context.put("controller", "manager");
		return new ModelAndView(analyzeSurveySectionTemplate, "context", context);
	}
	
	public ModelAndView showSurveyFile(HttpServletRequest request, HttpServletResponse response, ManageSurveyForm form, BindException errors) throws Exception {
	...
	public ModelAndView saveSurveySectionAnalizeResponse(HttpServletRequest request, HttpServletResponse response, ManageSurveyForm form, BindException errors) throws Exception {
		...
		return new ModelAndView("redirect:mgr_manageSurveys.do?method=showSurveyAnalizeForIndividuals&sid="+form.getSurvey().getSurveyRef().getId());
	}
	...
	public ModelAndView saveSurveyAnalizeResponse(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		if(surveyResult != null) {
			...
			for(int i =0; i<questionList.size(); i++) {
				...
				SurveyResultAnswer surveyResultAnswer = surveyService.getSurveyResultAnswerByQuestionIdAndVU360UserId(questionList.get(i).getSurveyQuestionRef().getId().longValue(), userId.longValue());
				if(surveyResultAnswer != null) {
	...
	public ModelAndView showManageFlags(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		ManageSurveyForm form = (ManageSurveyForm)command;
		try {
			List<SurveyFlagTemplate> surveyFlagTemplates = new ArrayList<SurveyFlagTemplate>();
			surveyFlagTemplates = surveyService.findAllFlagTemplate(form.getSearchedFlagName(),form.getSid());
	...
	/**
	 * handler methods for sort flags
	 */
	...
	public ModelAndView saveNewFlag(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		ManageSurveyValidator validator= (ManageSurveyValidator) getValidator();
	...
	public ModelAndView exportSurvey(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		if ( surveyResList!=null && surveyResList.size() > 0 ) {
			log.debug( surveyResList.size() + " Survey Results to be exported to Predict360 ");
			...
			log.debug( "No Survey Results were found for export!" );
		}
		return searchSurvey(request, response, command, errors);
	}
