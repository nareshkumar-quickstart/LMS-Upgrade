 ...
 * @author Dyutiman
 */
public class EditInstructorValidator implements Validator {

	private VU360UserService vu360UserService;
	private static final Logger log = Logger.getLogger(EditInstructorValidator.class.getName());
	...
	public void validate(Object obj, Errors errors) {
		...
		validateInstructor(form, errors);

	}

	public void validateSummaryPage(InstructorForm form, Errors errors) {
		...
		}/* else if( vu360UserService.getUsersByEmailAddress(form.getInstructor().getUser().
				getEmailAddress()).size() != 0 ) {
...
//		} else if ( form.getInstructor().getUser().getPassword().length() < 8 ) {
	...
	public void validateInstructor(InstructorForm form, Errors errors) {
		if ( StringUtils.isBlank(form.getInstId()) ) {
			errors.rejectValue("instId", "error.editApproval.instructor.required");
		}
	}
