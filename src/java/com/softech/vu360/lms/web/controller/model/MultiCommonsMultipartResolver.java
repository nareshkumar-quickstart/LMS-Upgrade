    ...
    protected MultipartParsingResult parseFileItems(List fileItems, String encoding) {
		...
		for (Iterator it = fileItems.iterator(); it.hasNext();) {
			...
			if (fileItem.isFormField()) {
                ...
                boolean hasLength = StringUtils.hasLength(file.getOriginalFilename());
                
				HttpServletRequest originalRequest = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
				String request = originalRequest.getRequestURI();
				MultipartHttpServletRequest multiPartRequest = new DefaultMultipartHttpServletRequest(originalRequest);

				if(request.equalsIgnoreCase("/lms/ins_addCourse.do") || request.equalsIgnoreCase("/lms/lrn_download.do") || request.equalsIgnoreCase("/lms/ins_viewCourse.do") &&  file != null)
				{
